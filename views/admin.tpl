...
<center>
    ...
    <tr>
     ...
     <td>{{$values.Role}}</td>
     <td>{{$values.Email}}</td>
     <td><button onclick="javascript:if(confirm('Are you sure you want to delete it?'))window.location.href='/admin/delete/{{$values.Userid}}'">Delete</button></td>
     <td><button onclick="window.location.href='/admin/update/{{$values.Userid}}'">Update</button></td>
     </tr>
     {{end}}
     <tr>
